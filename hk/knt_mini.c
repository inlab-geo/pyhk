...
Revision History:
  ...
  Jiawen He  2/20/2024 add Python wrapper partial_modified()
...
int partial_modified( int		ps,	/* 0=P; 1=S receiver function */
		int		nft,	/* number of pts for fft */ /* also the number of time series points */
		int		m,	/* number of input layers */
		const float	thik[],	/* thickness */
		const float	beta[], /* Vs */
		const float	kapa[],	/* Vp/Vs */
		...
		float 		db,	/* amount of Vs perturbation */
		float 		dh,	/* amount of thickness perturbation */
    float     *arr // size of dpred should equals to nft
  )
{
  int     	i, j, k, n, nft2, nlyrs;
  unsigned char	vORh;
  ...
  for (i=0,w=0.; i<nft2; i++,w+=delw) {
      ...
      temp = cphase(cmplx(-agg*agg, -shft*w));
      dis = rcvrfn(w, nlyrs, lyr);
      if (ps==0) dpred[i] = cmltp(dis.sp, cinvs(cmltp(IMAGE, dis.pp)));
      else dpred[i] = cngtv(conjg(cmltp(cmltp(IMAGE,dis.ps), cinvs(dis.ss))));
      dpred[i] = cmltp(temp, dpred[i]);
      if (db > 0.) {
         rcvrtd(w, nlyrs, lyr);
         for (k=i+nft2,vORh=0,j=1; j<nlyrs; j++,k+=nft2) {
	     dis = delrcv(w,lyr+j);
	     if (ps==0) dpred[k] = cmltp(dis.sp, cinvs(cmltp(IMAGE, dis.pp)));
	     else dpred[k] = cngtv(conjg(cmltp(cmltp(IMAGE,dis.ps), cinvs(dis.ss))));
             dpred[k] = cmltp(temp, dpred[k]);
	     dpred[k] = dmltp(delta[vORh],cplus(dpred[k], cngtv(dpred[i])));
	     vORh ^= 1;
         }
      }
  }
